import React from "react";
import {
  FaPhoneAlt,
  FaEnvelope,
  FaMapMarkerAlt,
  FaClock,
  FaPrayingHands,
} from "react-icons/fa";
import { useTranslation } from "react-i18next";

export const Footer = () => {
  const { t } = useTranslation();

  return (
    <footer className="bg-gray-900 text-gray-300 py-10 w-full border-t border-gray-800">
      <div className="max-w-7xl mx-auto px-6">
        {/* Grid: എല്ലാ ഐറ്റങ്ങളും മൊബൈലിൽ ഒരേപോലെ വരാൻ */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          
          {/* Section 1 */}
          <div className="flex flex-col items-start">
            <div className="flex items-center gap-3">
              <FaPrayingHands className="text-amber-400 text-2xl" />
              <h2 className="text-base font-bold text-white uppercase tracking-wider">
                {t("temple_full_name")}
              </h2>
            </div>
          </div>

          {/* Section 2: Contact Info */}
          <div className="flex flex-col items-start w-full">
            <h3 className="text-lg font-semibold text-amber-400 mb-4 border-b border-amber-400/30 pb-1 w-full">
              {t("contact_info")}
            </h3>
            <ul className="space-y-4 text-sm w-full">
              <li className="flex items-start gap-3">
                <FaPhoneAlt className="text-amber-400 mt-1 shrink-0" />
                <a href="tel:+917306327251" className="hover:text-amber-400 transition">
                  +91 73063 27251
                </a>
              </li>
              <li className="flex items-start gap-3">
                <FaEnvelope className="text-amber-400 mt-1 shrink-0" />
                <a href="mailto:pathaikkaramana@gmail.com" className="hover:text-amber-400 transition break-all">
                  pathaikkaramana@gmail.com
                </a>
              </li>
              <li className="flex items-start gap-3">
                <FaMapMarkerAlt className="text-amber-400 mt-1 shrink-0" />
                <span>
                  {t("temple_location") || "Pathaikkara Mana, Perinthalmanna, Kerala 679322, India"}
                </span>
              </li>
            </ul>
          </div>

          {/* Section 3: Timings */}
          <div className="flex flex-col items-start w-full">
            <h3 className="text-lg font-semibold text-amber-400 mb-4 border-b border-amber-400/30 pb-1 w-full">
              {t("temple_timings")}
            </h3>
            <div className="space-y-3 w-full">
              <div className="flex items-center gap-3 text-sm">
                <FaClock className="text-amber-400" />
                <span className="w-16">{t("morning")}</span>
                <span className="bg-gray-800 px-2 py-1 rounded text-white font-mono">6:00-8:30 AM</span>
              </div>
              <div className="flex items-center gap-3 text-sm">
                <FaClock className="text-amber-400" />
                <span className="w-16">{t("evening")}</span>
                <span className="bg-gray-800 px-2 py-1 rounded text-white font-mono">5:00-7:00 PM</span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
};